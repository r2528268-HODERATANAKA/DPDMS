    ...
    public void notifyApproved(ZoonoticIncident incident) {
        ...
        try {
            ...
            restClient.post()
                    ...
                    .retrieve()
                    .toBodilessEntity();
            log.info("Handed approval alert for zoonotic incident {} to alert-service", incident.getId());
        } catch (Exception ex) {
            log.warn("Could not reach alert-service (record is still approved): {}", ex.getMessage());
        }
    }

    private String buildMessage(ZoonoticIncident incident) {
        return String.format("[Zoonotic disease - %s] APPROVED incident #%d in ward %s, %s district. %s",
                incident.getSeverity(), incident.getId(), incident.getWard(), incident.getDistrict(),
                String.format("%s via %s: %d human cases, %d animals affected.", incident.getDiseaseName(), incident.getSuspectedAnimalSpecies(), incident.getHumanCases(), incident.getAnimalsAffected()));
    }
}
