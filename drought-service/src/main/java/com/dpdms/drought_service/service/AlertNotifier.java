    ...
    public void notifyApproved(DroughtIncident incident) {
        ...
        try {
            ...
            restClient.post()
                    ...
                    .retrieve()
                    .toBodilessEntity();
            log.info("Handed approval alert for drought incident {} to alert-service", incident.getId());
        } catch (Exception ex) {
            log.warn("Could not reach alert-service (record is still approved): {}", ex.getMessage());
        }
    }

    private String buildMessage(DroughtIncident incident) {
        return String.format("[Drought - %s] APPROVED incident #%d in ward %s, %s district. %s",
                incident.getSeverity(), incident.getId(), incident.getWard(), incident.getDistrict(),
                String.format("Rainfall deficit %.1f mm over %d dry days, %d households affected.", incident.getRainfallDeficitMm(), incident.getConsecutiveDryDays(), incident.getAffectedHouseholds()));
    }
}
